...
See? You've just created a Discord bot that can use slash commands! If you want to add message-based commands to your bot, replace the `@slash_command` decorator with the `@commands.command` decorator:

```python
import discord
from discord import commands # import the commands module
...
Now, explore pycord! It has many other features and functionality that I did not cover here. I recommend looking at its <a href='https://docs.pycord.dev/en/master/' target='_blank'>documentation</a> and checking out its <a href='https://github.com/Pycord-Development/pycord' target='_blank'>GitHub Repository</a> (it's open source, meaning you can contribute to it).
